...
$CASComputeQuota = New-ComputeQuota -QuotaName CAS-Compute -vmCount 100 -memoryLimitMB 102400 -coresLimit 200
$CASNetworkQuota = New-NetworkQuota -QuotaName CAS-Network -publicIpsPerSubscription 50 -vNetsPerSubscription 50 -gatewaysPerSubscription 10 -connectionsPerSubscription 20 -loadBalancersPerSubscription 50 -nicsPerSubscription 50 -securityGroupsPerSubscription 50
$CASStorageQuota = New-StorageQuota -QuotaName CAS-Storage -capacityInGb 100 -numberOfStorageAccounts 20
$ITComputeQuota = New-ComputeQuota -QuotaName CAS-IT-Compute -vmCount 250 -memoryLimitMB 200400 -coresLimit 175
$ITNetworkQuota = New-NetworkQuota -QuotaName CAS-IT-Network -publicIpsPerSubscription 75 -vNetsPerSubscription 75 -gatewaysPerSubscription 15 -connectionsPerSubscription 30 -loadBalancersPerSubscription 50 -nicsPerSubscription 75 -securityGroupsPerSubscription 75
$ITStorageQuota = New-StorageQuota -QuotaName CAS-IT-Storage -capacityInGb 175 -numberOfStorageAccounts 100
$DEVComputeQuota = New-ComputeQuota  -QuotaName CAS-Dev-Compute -vmCount 500 -memoryLimitMB 1024000 -coresLimit 350
$DEVNetworkQuota = New-NetworkQuota -QuotaName CAS-Dev-Network -publicIpsPerSubscription 100 -vNetsPerSubscription 100 -gatewaysPerSubscription 50 -connectionsPerSubscription 100 -loadBalancersPerSubscription 100 -nicsPerSubscription 500 -securityGroupsPerSubscription 500
$DEVStorageQuota = New-StorageQuota -QuotaName CAS-Dev-Storage -capacityInGb 300 -numberOfStorageAccounts 50
...
$CasFinPlan = New-AzureRmPlan -Name cas-fin-plan -DisplayName CAS-Finance-Plan -ArmLocation $ArmLocation -ResourceGroup  $OfferRG -QuotaIds @($CASComputeQuota.id,$CASNetworkQuota.id,$CASStorageQuota.id,$KVQuota.id)
$CasHrPlan = New-AzureRmPlan -Name cas-hr-plan -DisplayName CAS-HR-Plan -ArmLocation $ArmLocation -ResourceGroup  $OfferRG -QuotaIds  @($CASComputeQuota.id,$CASNetworkQuota.id,$CASStorageQuota.id)
$CasDevPlan = New-AzureRmPlan -Name cas-dev-plan -DisplayName CAS-DEV-Plan -ArmLocation $ArmLocation -ResourceGroup  $OfferRG -QuotaIds @($DEVComputeQuota.id,$DEVNetworkQuota.id,$DEVStorageQuota.id,$KVQuota.id)
$CasItPlan = New-AzureRmPlan -Name cas-it-plan -DisplayName CAS-IT-Plan -ArmLocation $ArmLocation -ResourceGroup  $OfferRG -QuotaIds @($ITComputeQuota.id,$ITNetworkQuota.id,$ITStorageQuota.id)
...
New-AzureRMOffer -name CAS-HR-offer -DisplayName CAS-HR-Offer -State Public -ARMLocation $ArmLocation -ResourceGroup $OfferRG -BasePlanIds $CasHrPlan.Id
New-AzureRMOffer -name CAS-FIN-offer -DisplayName CAS-Fin-Offer -State Public -ARMLocation $ArmLocation -ResourceGroup $OfferRG -BasePlanIds $CasFinPlan.Id
New-AzureRMOffer -name CAS-IT-offer -DisplayName CAS-IT-Offer -ARMLocation $ArmLocation -ResourceGroup $OfferRG -BasePlanIds $CasItPlan.Id
New-AzureRMOffer -name CAS-Dev-offer -DisplayName CAS-Dev-Offer -ARMLocation $ArmLocation -ResourceGroup $OfferRG -BasePlanIds $CasDevPlan.Id
